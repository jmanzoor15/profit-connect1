...
function emit$1(node, context, name, payload, bubble2 = true, meta2) {
  context._e(node, {
    payload,
    name,
    bubble: bubble2,
...
function nodeInit(node, options2) {
  var _a, _b, _f;
  const hasInitialId = (_a = options2.props) === null || _a === void 0 ? void 0 : _a.id;
  if (!hasInitialId)
    (_b = options2.props) === null || _b === void 0 ? true : delete _b.id;
  node.ledger.init(node.store._n = node.props._n = node.config._n = node);
  node.props._emit = false;
  Object.assign(node.props, hasInitialId ? {} : { id: `input_${idCount++}` }, (_f = options2.props) !== null && _f !== void 0 ? _f : {});
  node.props._emit = true;
  ...
  node.on("created", () => {
    ...
    node.context.handlers.resetFiles = (e) => {
      ...
      if (node.props.id && isBrowser) {
        ...
        if (el2)
          el2.value = "";
        el2 === null || el2 === void 0 ? void 0 : el2.focus();
      }
    };
...
function useSchema(inputSection) {
  return outer(wrapper(label("$label"), inner(icon("prefix"), prefix(), inputSection(), suffix(), icon("suffix"))), help("$help"), messages(message("$message.value")));
}
...
var checkbox = {
  ...
  schema: outer(
    $if(
       ...
       */
      boxWrapper(boxInner(prefix(), box(), decorator(icon("decorator")), suffix()), $extend(boxLabel("$label"), {
        if: "$label"
      })),
      /**
       * Multi checkbox structure.
       */
      fieldset(legend("$label"), help("$help"), boxOptions(boxOption(boxWrapper(boxInner(prefix(), $extend(box(), {
        bind: "$option.attrs",
        attrs: {
          id: "$option.attrs.id",
          value: "$option.value",
...
var file = {
   ...
   * The actual schema of the input, or a function that returns the schema.
   */
  schema: outer(wrapper(label("$label"), inner(icon("prefix", "label"), prefix(), fileInput(), fileList(fileItem(icon("fileItem"), fileName("$file.name"), $if("$value.length === 1", fileRemove(icon("fileRemove"), '$ui.remove.value + " " + $file.name')))), $if("$value.length > 1", fileRemove("$ui.removeAll.value")), noFiles(icon("noFiles"), "$ui.noFiles.value"), suffix(), icon("suffix"))), help("$help"), messages(message("$message.value"))),
  /**
   * The type of node, can be a list, group, or input.
   */
  type: "input",
...
var select2 = {
   ...
   * The actual schema of the input, or a function that returns the schema.
   */
  schema: outer(wrapper(label("$label"), inner(icon("prefix"), prefix(), selectInput$1($if("$slots.default", () => "$slots.default", optionSlot($if("$option.group", optGroup(optionSlot(option("$option.label"))), option("$option.label"))))), $if("$attrs.multiple !== undefined", () => "", icon("select")), suffix(), icon("suffix"))), help("$help"), messages(message("$message.value"))),
  /**
   * The type of node, can be a list, group, or input.
   */
  type: "input",
...
var validation$G = {
  ...
  date_after({ name, args }) {
    if (Array.isArray(args) && args.length) {
      return `${sentence(name)} mora biti poslije ${date2(args[0])}.`;
    }
   ...
   */
  ends_with({ name, args }) {
    return `${sentence(name)} ne završava sa ${list2(args)}.`;
  },
  /**
   * Is not an allowed value
   * @see {@link https://formkit.com/essentials/validation#is}
  ...
  length({ name, args: [first = 0, second = Infinity] }) {
    ...
    if (min3 == 0 && max3) {
      return `${sentence(name)} mora biti manje od ili jednako ${max3} karaktera.`;
    }
    if (min3 === max3) {
      return `${sentence(name)} treba imati ${max3} karaktera.`;
    }
    if (min3 && max3 === Infinity) {
      return `${sentence(name)} mora biti veći od ili jednak ${min3} karaktera.`;
    }
...
var validation$k = {
  ...
  date_after({ name, args }) {
    if (Array.isArray(args) && args.length) {
      return `${name} jābūt pēc ${date2(args[0])}.`;
    }
  ...
  date_before({ name, args }) {
    if (Array.isArray(args) && args.length) {
      return `${name} jābūt pirms ${date2(args[0])}.`;
    }
   ...
   */
  date_between({ name, args }) {
    return `${name} jābūt starp ${date2(args[0])} un ${date2(args[1])}`;
  },
  /**
   * Shown when the user-provided value is not a valid email address.
   * @see {@link https://formkit.com/essentials/validation#email}
   ...
   */
  ends_with({ name, args }) {
    return `${name} nebeidzas ar ${list2(args)}.`;
  },
  /**
   * Is not an allowed value
   * @see {@link https://formkit.com/essentials/validation#is}
  ...
  length({ name, args: [first = 0, second = Infinity] }) {
    ...
    if (min3 == 0 && max3) {
      return `${name} jābūt mazāk par vai ${max3} simboliem.`;
    }
    if (min3 === max3) {
      return `${name} jābūt ${max3} simbolu garumā.`;
    }
    if (min3 && max3 === Infinity) {
      return `${name} jābūt vismaz ${min3} simboliem.`;
    }
    return `${name} jābūt starp ${min3} un ${max3} simboliem.`;
  },
  /**
   * Value is not a match
   * @see {@link https://formkit.com/essentials/validation#matches}
   ...
   */
  starts_with({ name, args }) {
    return `${name} nesākas ar ${list2(args)}.`;
  },
  /**
   * Is not a url
   * @see {@link https://formkit.com/essentials/validation#url}
...
var validation$h = {
  ...
  length({ name, args: [first = 0, second = Infinity] }) {
    ...
    if (min3 == 0 && max3) {
      return `${sentence(name)} mag maximaal ${max3} tekens lang zijn.`;
    }
    if (min3 === max3) {
      return `${sentence(name)} moet ${max3} tekens lang zijn.`;
    }
    if (min3 && max3 === Infinity) {
      return `${sentence(name)} moet minimaal ${min3} tekens lang zijn.`;
    }
    return `${sentence(name)} moet tussen de ${min3} en ${max3} tekens zijn.`;
  },
  /**
   * Value is not a match
   * @see {@link https://formkit.com/essentials/validation#matches}
...
var validation$8 = {
  ...
  date_after({ name, args }) {
    if (Array.isArray(args) && args.length) {
      return `${sentence(name)} tenki depoid ${date2(args[0])}.`;
    }
  ...
  date_before({ name, args }) {
    if (Array.isArray(args) && args.length) {
      return `${sentence(name)} tenki antes ${date2(args[0])}.`;
    }
   ...
   */
  date_between({ name, args }) {
    return `${sentence(name)} tenki iha ${date2(args[0])} no ${date2(args[1])} nia klaran`;
  },
  /**
   * Shown when the user-provided value is not a valid email address.
   * @see {@link https://formkit.com/essentials/validation#email}
   ...
   */
  ends_with({ name, args }) {
    return `${sentence(name)} la remata ho ${list2(args)}.`;
  },
  /**
   * Is not an allowed value
   * @see {@link https://formkit.com/essentials/validation#is}
  ...
  length({ name, args: [first = 0, second = Infinity] }) {
    ...
    if (min3 == 0 && max3) {
      return `${sentence(name)} tenki badak liu ${max3} letra.`;
    }
    if (min3 === max3) {
      return `${sentence(name)} tenki iha letra ${max3}.`;
    }
    if (min3 && max3 === Infinity) {
      return `${sentence(name)} tenki iha letra ${min3} ka liu.`;
    }
    return `${sentence(name)} tenki iha letra ${min3} too ${max3}.`;
  },
  /**
   * Value is not a match
   * @see {@link https://formkit.com/essentials/validation#matches}
   ...
   */
  starts_with({ name, args }) {
    return `${sentence(name)} la komesa ho ${list2(args)}.`;
  },
  /**
   * Is not a url
   * @see {@link https://formkit.com/essentials/validation#url}
...
var formkitComponent = defineComponent(FormKit, {
  props: runtimeProps,
  inheritAttrs: false
});
...
async function fetchInputSchema(input2) {
  try {
    const response = await fetch(`https://raw.githubusercontent.com/formkit/input-schemas/master/schemas/${input2}.json`);
    const json = await response.json();
    return json;
  } catch (error2) {
...
var FormKitKitchenSink = defineComponent({
  ...
  async setup(props) {
    ...
    const filteredFormNames = formNames.filter((form2) => form2.name !== "");
    ...
    const formRenders = computed(() => {
      return filteredFormNames.map((form2) => {
        const schemaDefinition = schemas[form2.id];
        return h("div", {}, activeTab.value === form2.id ? [
          h("div", {
    ...
    return () => {
      return h("div", {
        ...
        }, kitchenSinkRenders.length && activeTab.value === "kitchen-sink" ? kitchenSinkRenders : []),
        ...formRenders.value
      ]);
    };
  }
...
var definition$1 = messages$1(message$1("$message.value"));
...
var FormKitProvider = defineComponent(function FormKitProvider2(props, { slots }) {
  const options2 = {};
  if (props.config) {
    useConfig(props.config);
  }
  return () => slots.default ? slots.default(options2) : null;
}, { props: ["config"], name: "FormKitProvider" });
var FormKitConfigLoader = defineComponent(async function FormKitConfigLoader2(props, context) {
  var _a;
  let config = {};
  if (props.configFile) {
  ...
  if (useDefaultConfig) {
    const { defaultConfig: defaultConfig3 } = await Promise.resolve().then(function() {
      return defaultConfig$1;
    });
    config = defaultConfig3(config);
  }
  return () => h(FormKitProvider, { config }, context.slots);
}, {
  props: ["defaultConfig", "configFile"]
});
var FormKitLazyProvider = defineComponent(function FormKitLazyProvider2(props, context) {
...
var definition = summary(summaryInner(summaryHeader("$summaryHeader"), messages2(message2(messageLink("$summary.message")))));
var FormKitSummary = defineComponent({
  ...
  setup(props, context) {
    ...
    const addContexts = () => {
      ...
      (_a2 = node.value) === null || _a2 === void 0 ? void 0 : _a2.walk((child) => child.context && summaryContexts.value.push(child.context));
    };
    (_a = node.value) === null || _a === void 0 ? void 0 : _a.on("submit-raw", async () => {
      ...
      await nextTick();
      if (typeof window !== "undefined") {
        (_a2 = document.getElementById(id2)) === null || _a2 === void 0 ? void 0 : _a2.scrollIntoView({ behavior: "smooth" });
        if (summaries.value[0]) {
          (_b2 = document.getElementById(summaries.value[0].key)) === null || _b2 === void 0 ? void 0 : _b2.focus();
        }
      }
    });
    ...
    function jumpLink(e) {
      ...
      if (e.target instanceof HTMLAnchorElement) {
        e.preventDefault();
        const id3 = (_a2 = e.target.getAttribute("href")) === null || _a2 === void 0 ? void 0 : _a2.substring(1);
        if (id3) {
          (_b2 = document.getElementById(id3)) === null || _b2 === void 0 ? void 0 : _b2.scrollIntoView({ behavior: "smooth" });
          (_c = document.getElementById(id3)) === null || _c === void 0 ? void 0 : _c.focus();
        }
      }
    }
    ...
    const data = computed(() => {
      ...
      return {
        id: id2,
        fns: ((_b2 = (_a2 = node.value) === null || _a2 === void 0 ? void 0 : _a2.context) === null || _b2 === void 0 ? void 0 : _b2.fns) || {},
    ...
    return () => {
      var _a2;
      return ((_a2 = node.value) === null || _a2 === void 0 ? void 0 : _a2.context) ? h(FormKitSchema, { schema, data: data.value }, { ...context.slots }) : null;
    };
  }
});
...
var defaultConfig$1 = Object.freeze({
  __proto__: null,
